...
# descrição: Efetua a limpeza de pacotes APT e atualiza os pacotes em outros formatos, num pós-instalação
# version: 1.0
# licença: MIT License
...
echo ${senha} | (sudo -S apt --fix-broken install -y && sudo -S apt list --upgradable && sudo -S apt update -y && sudo -S apt dist-upgrade -y && sudo -S apt install -f && sudo -S apt autoremove -y && sudo -S apt autoclean -y && sudo -S snap refresh && flatpak update -y)
echo ""
